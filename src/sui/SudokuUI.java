...
public class SudokuUI extends Application
{
    ...
    {
        so = new SudokuOptions();
        sg = new SudokuGenerator(16, 16);
    ...
    /**
     * Initializes the puzzlePane and sets the eventHandlers and other properties
     * of the puzzle textFields.  There is one SudokuTextField for each element
     * of the array, so 81 items in total by default.  Borders are also set here
     * based on the physical location of the SudokuTextField, so that the entire
     * collection resembles an actual Sudoku puzzle.
     */
    private void initializePuzzlePane()
    {
        ...
        for (int i = 0; i < SudokuGenerator.MAX_VALUE; i++)
        {
            ...
            {
                SudokuTextField tf = new SudokuTextField();
                tf.setPrefHeight(50);
                tf.setPrefWidth(50);
                tf.setAlignment(Pos.CENTER);
                tf.setFont(Font.font(STYLESHEET_MODENA, FontWeight.NORMAL, 16));
                tf.setEditable(true);
                ...
                tf.textProperty().addListener((ov, oldV, newV) ->{ 
                                    ...
                                    if (!oldV.equals(newV)) 
                                    {
                                        ...
                                        leaveFocus(index, sg.getDisplay(), row, col); 
                ...
                tf.focusedProperty().addListener((fov) -> { });
                ...
                {
                    tf.setBorderThickness(0, 8, 8, 0, 8);
                    
                }
                else if (col == SudokuGenerator.MAX_VALUE - 1 && !(row % 3 == 0))
                {
                    tf.setBorderThickness(0, 8, 0, 0, 8);
                    
                }
                else if (row == SudokuGenerator.MAX_VALUE - 1 && !(col % 3 == 0))
                {
                    tf.setBorderThickness(0, 0, 8, 0, 8);
                    
                }
                else if (row == SudokuGenerator.MAX_VALUE - 1 && col % 3 == 0)
                {
                    tf.setBorderThickness(0, 0, 8, 8, 8);
                    
                }
                else if (col == SudokuGenerator.MAX_VALUE - 1 && row % 3 == 0)
                {
                    tf.setBorderThickness(8, 8, 0, 0, 8);
                    
                }
                
                
                // Set the format to what we've already done.
                tf.setStyle(tf.getBorderFormat());
                ...
                tf.setOnKeyPressed((e) ->
                {
                    ...
                    switch(e.getCode())
                    {
                        case UP: { keyUp(index); break; }
                        case DOWN: { keyDown(index); break; }
                        case LEFT: { keyLeft(index); break; }
                        case RIGHT: { keyRight(index); break; }
    ...
    /**
     * Initializes the buttons, click event handler methods, and their storage pane
     * before adding the controls to the main scene frame.
     */
    private void initializeFuctionalButtons()
    {
        ...
        bCommit.setOnAction(commitClickEvent -> { onCommitClickEvent(); });
        
        
        Button bSurrender = new Button();
        bSurrender.setText("Surrender Button");
        bSurrender.setOnAction(surrenderClickEvent -> { onSurrenderClickEvent(); });
    ...
    /** 
     * This is the hub for initial puzzle display.  It calls to the generator
     * to make a puzzle, seeding it with sixteen random numbers (by default) at 
        ...
        for (row = 0; row < SudokuGenerator.MAX_VALUE; row++)
        {
            for (col = 0; col < SudokuGenerator.MAX_VALUE; col++)
            {
                ...
                if (puzzlePane.getChildren().get(col) instanceof TextField)
                {
                    ...
                    ((TextField)puzzlePane.getChildren().get(row * 9 + col)).setText(Integer.toString(sg.getDisplay()[row][col]));
    ...
    public void debugShowUser()
    {
        ...
        for (row = 0; row < SudokuGenerator.MAX_VALUE; row++)
        {
            for (col = 0; col < SudokuGenerator.MAX_VALUE; col++)
            {
                ...
                {
                    // Set the text for hints.  This is done by converting the child-node index from the matrix index, so row * 9 + col.
                    ((TextField)puzzlePane.getChildren().get(row * 9 + col)).setText(Integer.toString(sg.getSolution()[row][col]));
    ...
    public void leaveFocus(int index, int[][] array, int row, int col)
    {
        ...
        if (!sg.isSafeBlock(array, val, row, col) || !sg.isSafeColOrRow(array, val, row, col) || !checkIfValidData(val))
        {
     ...
     * or do not violate rules, the comparison with the solution array occurs, and
     * if no errors are detected, the method displays an alert that indicates
     * victory.  The elapsed timer is also stopped.
     */
    public void onCommitClickEvent() 
    {
        ...
        for (int row = 0; row < sg.getSolution().length; row++)
        {
            for (int col = 0; col < sg.getSolution()[row].length; col++)
            {
                // DEBUG PRINTOUT
                ...
                if (Integer.parseInt(((SudokuTextField)puzzlePane.getChildren().get(row * 9 + col)).getText()) != sg.getSolution()[row][col])
                {
     ...
     * The user is chastised for a quitter's mentality.  The elapsed timer is also
     * shut off.
     */
    ...
    /**
     * This handles the key-down event and moves the focus to the SudokuTextField
     * directly below the recipient of the event.
     * @param index The index of the STF that currently has focus and which 
     * received the event.
     */
    ...
    /**
     * This handles the key-left event and moves the focus to the SudokuTextField
     * directly left of the recipient of the event.
     * @param index The index of the STF that currently has focus and which 
     * received the event.
     */
    ...
    /**
     * This handles the key-right event and moves the focus to the SudokuTextField
     * directly right of the recipient of the event.
     * @param index The index of the STF that currently has focus and which 
     * received the event.
     */
